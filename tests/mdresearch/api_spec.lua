...
describe("yank-link command arguments", function()
  it("reads a bare register, and format= in any order", function()
    expect(commands.parse_link_args("")).eq({})
    expect(commands.parse_link_args("+")).eq({ register = "+" })
    expect(commands.parse_link_args("format=wiki")).eq({ format = "wiki" })
    expect(commands.parse_link_args("format=path +")).eq({ format = "path", register = "+" })
    expect(commands.parse_link_args("z format=wiki")).eq({ format = "wiki", register = "z" })
    expect(commands.parse_link_args("register=+")).eq({ register = "+" })
  end)

  it("rejects an argument it cannot place", function()
    local _, err = commands.parse_link_args("wiki")
    expect(err).contains("neither a register nor")
    local _, err2 = commands.parse_link_args("style=wiki")
    expect(err2).contains("unknown argument")
  end)
end)
...
describe("results actions", function()
  ...
  it("yanks a markdown link relative to the workspace", function()
    local buf = table_of({ tags = "journal" })
    vim.fn.setreg('"', "")
    expect(md.results.yank_link({ buf = buf, index = 1 })).truthy()
    expect(vim.fn.getreg('"')).eq("[Journal entry](journal/2025-01-02.md)")
  end)

  it("honours the format and register the caller names", function()
    local buf = table_of({ tags = "journal" })
    expect(md.results.yank_link({ buf = buf, index = 2, format = "path", register = "z" })).truthy()
    expect(vim.fn.getreg("z")).eq("journal/2025-12-31.md")
    expect(md.results.yank_link({ buf = buf, index = 2, format = "wiki", ext = false, register = "z" })).truthy()
    expect(vim.fn.getreg("z")).eq("[[journal/2025-12-31]]")
  end)

  it("keeps ext = false from the caller over the configured true", function()
    local buf = table_of({ tags = "journal" })
    expect(results.link_of({ buf = buf, index = 2, format = "path", ext = false })).eq("journal/2025-12-31")
    expect(results.link_of({ buf = buf, index = 2, format = "path" })).eq("journal/2025-12-31.md")
  end)

  it("yanks charwise, so the paste lands inline", function()
    local buf = table_of({ tags = "journal" })
    md.results.yank_link({ buf = buf, index = 1, register = "z" })
    expect(vim.fn.getregtype("z")).eq("v")
  end)

  it("reports an empty line rather than yanking the wrong link", function()
    local buf = table_of({ tags = "journal" })
    local win = vim.fn.win_findbuf(buf)[1]
    vim.api.nvim_win_set_cursor(win, { 1, 0 })
    local ok, err = results.yank_link_row({ buf = buf })
    expect(ok).falsy()
    expect(err).contains("no file on this line")
  end)

  it("reports an unknown format instead of yanking", function()
    local buf = table_of({ tags = "journal" })
    local ok, err = results.yank_link_row({ buf = buf, index = 1, format = "org" })
    expect(ok).falsy()
